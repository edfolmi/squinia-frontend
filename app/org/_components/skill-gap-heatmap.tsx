...
import { ORG_COHORTS, SKILL_LABELS, skillGapForCohort } from "../_lib/org-mock-data";
...
/** Rows = skill dimensions, columns = cohorts. Cell = target − cohort average (larger = hotter gap). */
export function SkillGapHeatmap({ drillBasePath = "/org/analytics" }: Props) {
  return (
    <div className="overflow-x-auto rounded-2xl border border-[var(--rule)] bg-[var(--surface)]">
        ...
        <tbody>
          {DIMS.map((d) => (
            <tr key={d} className="border-b border-[var(--rule)] last:border-0">
              ...
              {ORG_COHORTS.map((c) => {
                const gapMap = skillGapForCohort(c.id);
                const gap = gapMap?.[d] ?? 0;
                return (
      ...
      <p className="border-t border-[var(--rule)] px-3 py-2 text-[11px] text-[var(--muted)]">
        Values are skill gaps (target minus cohort average). Lower is better. Click a cohort header to filter analytics.
      </p>
    </div>
  );
}
